...
class TimeSeriesFeatureGenerator:
    ...
    def _generate_window_features(self, data: pd.DataFrame, value_column: str, window_size: int):
        """
        ...
        """
        data[f'{value_column}_min_{window_size}'] = data[value_column].rolling(window=window_size).min()
        data[f'{value_column}_max_{window_size}'] = data[value_column].rolling(window=window_size).max()
        data[f'{value_column}_avg_{window_size}'] = data[value_column].rolling(window=window_size).mean()
        data[f'{value_column}_nth_{window_size}'] = data[value_column].shift(window_size - 1)
